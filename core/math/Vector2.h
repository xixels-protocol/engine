...
public:
	...
	inline static Vector2 Lerp( const Vector2& v1, const Vector2& v2, float f )
...
public:
	...
	inline _bool Equal( const Vector2& v, float margin = Math::cEpsilon ) const
		{ return Math::Abs( x - v.x ) <= margin && Math::Abs( y - v.y ) <= margin; }

	inline Vector2& Absolute( )
		{ x = Math::Abs( x ); y = Math::Abs( y ); return *this; }
	inline Vector2& Normalize( )
		{ float m = Magnitude( ); if ( m > 0.0f ) { x /= m; y /= m; } return *this; }
